...
export default function AppEmbed({ applications }: { applications: AppDef[] }) {
  ...
  return (
    <>
  <section id="portfolio" className="w-full max-w-[1200px] mx-auto py-12 sm:py-16">
        <h3 className="text-xl sm:text-2xl font-bold mb-8 sm:mb-12 text-center text-white tracking-wider">
          [ APPLICATIONS ]
        </h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {applications.map((app, idx) => (
            <div
              ...
              className={`tilt-card cursor-pointer bg-black border-2 border-cyan-400 p-4 sm:p-6 hover:border-white transition hover:shadow-lg hover:shadow-white/50 hover:bg-gray-950 ${idx % 3 === 0 ? 'nb-accent-cyan' : idx % 3 === 1 ? 'nb-accent-yellow' : 'nb-accent-magenta'}`}
            >
